...
// Admin user ID for audit logs (placeholder - will use first admin if exists)
...
// Audit log entries reflecting realistic admin actions
...
export async function seedAdminData(): Promise<{ success: boolean; message: string; counts: Record<string, number> }> {
  ...
  try {
    ...
    // 3. Insert marketplace offers
    const { error: offerError } = await supabase
      .from('marketplace_offers')
      .upsert(marketplaceOffers);
    
    if (offerError) throw new Error(`Offers: ${offerError.message}`);
    counts.offers = marketplaceOffers.length;
    ...
    counts.auditLogs = auditLogEntries.length;

    return {
      success: true,
      message: `Seeded ${counts.organizations} orgs, ${counts.vendors} vendors, ${counts.offers} offers, ${counts.auditLogs} audit logs`,
      counts,
    };
  } catch (error) {
    return {
...
export async function clearSeedData(): Promise<{ success: boolean; message: string }> {
  try {
    ...
    await supabase.from('audit_logs').delete().in('resource_id', [ORG_AD_ID, ORG_DXB_ID, ORG_SUSPENDED_ID, ...VENDOR_IDS]);
    await supabase.from('marketplace_offers').delete().in('vendor_id', VENDOR_IDS);
    await supabase.from('vendors').delete().in('id', VENDOR_IDS);
    await supabase.from('organizations').delete().in('id', [ORG_AD_ID, ORG_DXB_ID, ORG_SUSPENDED_ID]);
