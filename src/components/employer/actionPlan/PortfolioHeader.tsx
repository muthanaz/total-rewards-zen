 ...
 * Portfolio Header
 * 
 * PROMPT 07: Executive summary with impact ranges and reconciliation.
 * Shows total impact range (min–max), overdue count, and top blockers.
 */
...
export function PortfolioHeader({ metrics }: PortfolioHeaderProps) {
  const { 
    totalExpectedImpactAEDMin, 
    totalExpectedImpactAEDMax, 
    actionsOverdue, 
    actionsByStatus, 
    topBlockedReasons,
    completionRate,
  ...
  const showRange = totalExpectedImpactAEDMin !== totalExpectedImpactAEDMax;
  
  return (
    <div className="space-y-4">
      {/* Primary KPIs */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {/* Total Expected Impact - Range Display */}
                ...
                {showRange ? (
                  <>
                    <p className="text-xl font-bold text-success tabular-nums">
                      {formatCurrencyAED(totalExpectedImpactAEDMin, { abbreviate: true })}–{formatCurrencyAED(totalExpectedImpactAEDMax, { abbreviate: true })}
                    </p>
                    <p className="text-xs text-muted-foreground">Expected Impact Range</p>
                  </>
                ) : (
                  <>
                    <p className="text-2xl font-bold text-success tabular-nums">
                      {formatCurrencyAED(totalExpectedImpactAEDMax, { abbreviate: true })}
                    </p>
      ...
      {/* Top Blocked Reasons with AED at Risk */}
        ...
        <Card className="bg-warning/5 border-warning/20">
          <CardContent className="py-3">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-warning">Top Blocked Reasons</p>
